...
def run_logreg_final_pipeline(
    ...
    df_grid_all.to_csv(f"{save_dir}/{disease}_logreg_gridsearch_results.csv", index=False)
    df_final.to_csv(f"{save_dir}/{disease}_logreg_validation_results.csv", index=False)
    pd.DataFrame([best_row]).to_csv(
        f"{save_dir}/{disease}_logreg_best_model.csv", index=False
    )

    print(f"\n>>> [{disease}] Logistic Regression save complete")
...
if __name__ == "__main__":
    ...
    for disease in disease_list:
        ...
        res = run_logreg_final_pipeline(X, y, disease, SAVE_DIR)
